		...
		if(mysql_num_rows($query) == 0){	//ini artinya jika data hasil query di atas kosong
			...
			while($data = mysql_fetch_assoc($query)){	//perulangan while dg membuat variabel $data yang akan mengambil data di database
				...
				echo '<tr>';
                    ...
                    echo '<td><a href="?page=edit&id='.$data['id_user'].'">Edit</a> / <a href="?page=hapususer&id='.$data['id_user'].'" onclick="return confirm(\'Yakin?\')">Hapus</a></td>';	//menampilkan link edit dan hapus dimana tiap link terdapat GET id -> ?id=id_user
				echo '</tr>';
